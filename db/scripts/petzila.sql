...
CREATE TABLE IF NOT EXISTS `petzila`.`user` (
  ...
  `temporal_password_expiration` TIMESTAMP NULL,
...
-- -----------------------------------------------------
-- Table `petzila`.`publish_point`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `petzila`.`publish_point` ;

CREATE TABLE IF NOT EXISTS `petzila`.`publish_point` (
  `publish_point_id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `os` VARCHAR(50) NOT NULL,
  `publish_point` VARCHAR(1000) NOT NULL,
  `session_request_id` BIGINT UNSIGNED NOT NULL,
  PRIMARY KEY (`publish_point_id`),
  CONSTRAINT `fk_publish_point_session_request1`
    FOREIGN KEY (`session_request_id`)
    REFERENCES `petzila`.`session_request` (`session_request_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE UNIQUE INDEX `publish_point_id_UNIQUE` ON `petzila`.`publish_point` (`publish_point_id` ASC);

CREATE INDEX `fk_publish_point_session_request1_idx` ON `petzila`.`publish_point` (`session_request_id` ASC);
